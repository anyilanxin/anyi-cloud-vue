...
var labelRequired = function () {
  function check(node, reporter) {
    ...
    if (isAny$3(node, ['bpmn:FlowNode', 'bpmn:SequenceFlow', 'bpmn:Participant', 'bpmn:Lane'])) {
...
function uniqueBy(extractor) {
  ...
  for (
    var _len = arguments.length, collections = new Array(_len > 1 ? _len - 1 : 0), _key = 1;
    _key < _len;
    _key++
  ) {
  ...
  var result = map(grouped, function (val, __key) {
    return val[0];
  });
  return result;
}
    ...
    rootElements.map((rootElement) => {
      ...
      const elements = flatten(
        [].concat(
          ...
          (rootElement.laneSets &&
            getAllBpmnElements(rootElement.laneSets[0].lanes.filter(hasChildLaneSet))) ||
            [],
          (rootElement.childLaneSet && rootElement.childLaneSet.lanes) || [],
          (rootElement.childLaneSet &&
            getAllBpmnElements(rootElement.childLaneSet.lanes.filter(hasChildLaneSet))) ||
            []
        )
      );
...
var noDisconnected = function () {
  function check(node, reporter) {
    if (
      !isAny$2(node, ['bpmn:Task', 'bpmn:Gateway', 'bpmn:SubProcess', 'bpmn:Event']) ||
